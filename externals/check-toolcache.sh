...
is_dotnet_package_cached() {
    eval $invocation

    local relative_path_to_package="$1"
    local specific_version="${2//[$'\t\r\n']}"

    local dotnet_toolcache_root="${AGENT_TOOLSDIRECTORY}/dotnet"
    if [ ! -d "$dotnet_toolcache_root" ]; then
        say_verbose "is_dotnet_package_cached: $dotnet_toolcache_root not available"
        return 1
    fi
    local dotnet_toolcache_ver="$(combine_paths "$dotnet_toolcache_root" "$specific_version")"
    local dotnet_toolcache_path="$(combine_paths "$dotnet_toolcache_ver" "x64")"
    local dotnet_toolcache_complete="$(combine_paths "$dotnet_toolcache_ver" "x64.complete")"
    say_verbose "is_dotnet_package_cached: dotnet_toolcache_path=$dotnet_toolcache_path"
    if [ -d "$dotnet_toolcache_path" ] && [ -f "$dotnet_toolcache_complete" ]; then
        say_verbose "is_dotnet_package_cached: pkg found in $dotnet_toolcache_path"
        say "cache_dir=$dotnet_toolcache_path"
    else
        say_verbose "is_dotnet_package_cached: pkg not available"
        say_verbose "is_dotnet_package_cached: mkdir $dotnet_toolcache_path"
        mkdir -p "$dotnet_toolcache_path"
        say "cache_dir=$dotnet_toolcache_path"
    fi
    return 0
}
...
# This function is almost the same as generate_regular_links(),
# except that it only extracts the version and returns the cached dir of the pkg.
get_cache_dir() {
    # use only first feed
    local feed="${feeds[0]}"
    ...
    if is_dotnet_package_cached "$asset_relative_path" "$effective_version"; then
        say "$asset_name with version '$effective_version' is cached."
    fi
    return 0
}
